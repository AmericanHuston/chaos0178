...
@Disabled
@Autonomous(name = "SpecX", group = "PedroAutos")
public class SpecX extends OpMode {
    ...
    private final Point HangSpecimen1Point = new Point(36.5, 75);
    ...
    private final Pose sample1 = new Pose(24.5, 23, Math.toRadians(0));
    private final Point sample1Point = new Point(24.5, 23);
    private final Pose sample2 = new Pose(24.5, 13, Math.toRadians(0));
    private final Point sample2Point = new Point(24.5, 13);
    private final Pose sample3 = new Pose(45.5, 14.8, Math.toRadians(270));
    private final Point sample3Point = new Point(45.5, 14.8);
    private final Point SpecPrepStep1Point = new Point(25, 20);
    private final Pose SpecPrepStep1 = new Pose(25, 20, Math.toRadians(180));
    private final Pose SpecPrepStep2 = new Pose(25, 20, Math.toRadians(90));
    ...
    private PathChain SpecCollect3;
    private PathChain BlockToBase1;
    private PathChain BlockToBase2;
    private PathChain BlockToBase3;
    ...
    public void init() {
        ...
        SpecCollect1 = follower.pathBuilder()
                .addBezierLine(HangSpecimen1Point, sample1Point)
                .setLinearHeadingInterpolation(HangSpecimen1.getHeading(), sample1.getHeading())
                .build();
        SpecCollect2 = follower.pathBuilder()
                .addBezierLine(SpecPrepStep1Point, SpecPrepStep2Point)
                .setLinearHeadingInterpolation(SpecPrepStep1.getHeading(), SpecPrepStep2.getHeading())
                .addBezierLine(SpecPrepStep2Point, sample2Point)
                .setLinearHeadingInterpolation(SpecPrepStep2.getHeading(), sample2.getHeading())
                .build();
        SpecCollect3  = follower.pathBuilder()
                .addBezierLine(SpecPrepStep1Point, SpecPrepStep2Point)
                .setLinearHeadingInterpolation(SpecPrepStep1.getHeading(), SpecPrepStep2.getHeading())
                .addBezierLine(SpecPrepStep2Point, sample3Point)
                .setLinearHeadingInterpolation(SpecPrepStep2.getHeading(), sample3.getHeading())
                .build();
        BlockToBase1 = follower.pathBuilder()
                .addPath(new BezierLine(new Point(sample1), new Point(SpecPrepStep1)))
                .setLinearHeadingInterpolation(sample1.getHeading(), SpecPrepStep1.getHeading())
                .build();
        BlockToBase2 = follower.pathBuilder()
                .addPath(new BezierLine(new Point(sample2), new Point(SpecPrepStep1)))
                .setLinearHeadingInterpolation(sample2.getHeading(), SpecPrepStep1.getHeading())
                .build();
        BlockToBase3 = follower.pathBuilder()
                .addPath(new BezierLine(new Point(sample3), new Point(SpecPrepStep1)))
                .setLinearHeadingInterpolation(sample3.getHeading(), SpecPrepStep1.getHeading())
                .build();
    ...
    public void loop() {
        ...
        switch (autoState) {
            ...
            case 5:
                ...
                break;
            case 6: //goes to collect position
                if (!follower.isBusy()) {
                ...
                break;
            case 7: //closes the claw
                if (state_timer.getElapsedTimeSeconds() > 1.6) {
                    robot.closeMiniClaw();
                    robot.closeClaw();
                    if (state_timer.getElapsedTimeSeconds() > 1.7) {next_state();}
                }
                break;
            case 8:
            ...
            case 13:
                ...
                next_state();
                break;
            case 14://rotates to base
                if(!follower.isBusy() && state_timer.getElapsedTimeSeconds() > 0.5){
                    follower.followPath(BlockToBase2);
                    next_state();
                }
                break;
            case 15: //releases the claw
                if (!follower.isBusy()) {
                    robot.openClaw();
                    robot.openMiniClaw();
                    next_state();
                ...
                break;
            case 16: //turns to the blocks
                if(!follower.isBusy()){
                    follower.followPath(SpecCollect3);
                    robot.wristVertical();
                    robot.wristAct();
                    next_state();
                }
            ...
            case 18:
                ...
                next_state();
                break;
            case 19://rotates to base
                if(!follower.isBusy() && state_timer.getElapsedTimeSeconds() > 0.5){
                    follower.followPath(BlockToBase3);
                    next_state();
                }
                break;
            case 20: //releases the claw
                if (!follower.isBusy()) {
                    robot.openClaw();
                    robot.openMiniClaw();
                    next_state();
